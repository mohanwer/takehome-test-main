...
router.get(
  ...
  wrapAsyncRoute(async (req: Request, res: Response) => {
    ...
    const searchedKeyValues: [searchField: SearchFields, searchValue: string][] = []
    for (const [searchField, searchValue] of Object.entries(reqQuery)) {
      if (!searchValue) {
        continue;
      }

      searchDbForField(query, searchField as SearchFields, searchValue as string);
      searchedKeyValues.push([(searchField as SearchFields), searchValue as string])
    }
