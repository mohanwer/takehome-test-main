...
const wordConfidence = (word1: string, word2: string, i: number, j: number, matches: number): number => {
  if (i > word1.length - 1 || j > word2.length - 1) {
    return matches;
  }
  if (word1[i].toLowerCase() !== word2[j].toLowerCase()) {
    return Math.max(wordConfidence(word1, word2, i, j + 1, matches), wordConfidence(word1, word2, i + 1, j, matches));
  }
...
export const rowConfidence = (
  ...
  for (const [searchField, searchValue] of searchParams){
    ...
    let searchI = 0, rowI = 0;

    confidenceSummation += wordConfidence(searchValue, rowValue, searchI, rowI, 0);
